<nav class="navbar navbar-expand-lg navbar-light px-4  py-3 py-lg-0">
  ...
  <ul class="dropdown-menu">
     @forelse ($departments as $department)
                        <li class="mx-2" data-filter="{{ $department->id }}">
...
<ul class="dropdown-menu">
    @forelse ($departments as $department)
        <li class="mx-2">
            <!-- Department Link -->
            <a class="dropdown-item fw-bold" href="/department/{{ $department->id }}">
                {{ $department->title }}
            </a>

            <!-- Nested Courses List -->
            @if($department->courses->isNotEmpty())
                <ul class="dropdown-menu sub-menu ps-3"> 
                    <!-- 'sub-menu' is a custom class you might need to style, 
                         or rely on default browser nesting -->
    ...
    @empty
        <li class="dropdown-item text-muted">No departments found</li>
    @endforelse
    <hr/>
</li>
   <li class="nav-item dropdown">
